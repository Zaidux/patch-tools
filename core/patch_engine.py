...
class PatchEngine:
    ...
    def validate_patch(self, patch: Dict[str, Any], file_info: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if patch_type == 'insert_at_line':
            line_num = patch.get('line_number', 0)
            ...
            if start < 1 or end > file_info['lines'] or start > end:
                return False, f"Invalid line range: {start}-{end}"
                
        elif patch_type in ['replace_pattern', 'replace_pattern_all', 'insert_after', 'insert_before']:
            pattern = patch.get('pattern', '') or patch.get('after', '') or patch.get('before', '')
            if not pattern:
                return False, "Missing pattern for pattern-based patch"
